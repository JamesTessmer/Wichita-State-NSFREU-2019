...
class TaxoTree(object):
    ...
    def load_genomes(self, genomes_path=None):
        ...
        """
        also loads genus labels as a dictionary corresponding to genome
        """
        self.num_labels = 1050 #remove this, used to limit data to help run time as proof of concept
        ...
        #loading dictionary of genus to pkl file name. Done for both pathogenic and non-pathogenic genomes
        self.genus_to_pkl={}
        with open('ncbi_ids.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter = ';')
            for row in csv_reader:
                #print(row)
                pkl_name = row[0]
                pkl_name = pkl_name.split('.')[0]
                genus_and_species = row[2]
                genus = genus_and_species.split(' ')[0]
                self.genus_to_pkl[pkl_name]=[genus]
        with open('ncbi_id_pathogens.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter = ';')
            for row in csv_reader:
                #print(row)
                pkl_name = row[0]
        ...
        #list for reference when making labels
        self.genome_to_genus = [None]*self.num_labels
            ...
            if label < self.num_labels: 
                if genomes_path:
                    ...
                    with open(os.path.join(genomes_path, 
                        ...
                        #print(self.genomes[label])
        

                        #adding genome to genome_to_genus dictionary
                        self.genome_to_genus[label] = self.genus_to_pkl[name_load]

                current_path = self.id_to_path[taxo_id][1]
                #print(current_path)
                #print("current path printed")
                ...
                #print(self.paths[label])
                self.paths_full[label] = current_path_full 

        #print(self.genome_to_genus)
